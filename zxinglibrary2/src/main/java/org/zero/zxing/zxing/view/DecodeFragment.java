package org.zero.zxing.zxing.view;
...
import com.google.zxing2.client.result.ResultParser;

import org.zero.zxing.zxing.AmbientLightManager;
import org.zero.zxing.zxing.BeepManager;
import org.zero.zxing.zxing.FinishListener;
import org.zero.zxing.zxing.InactivityTimer;
import org.zero.zxing.zxing.IntentSource;
import org.zero.zxing.zxing.camera.CameraManager;
import org.zero.zxing.zxing.decode.CaptureFragmentHandler;
 ...
 */
public  class DecodeFragment extends Fragment implements
        SurfaceHolder.Callback, View.OnClickListener {


    private static final String TAG = DecodeFragment.class.getSimpleName();
    ...
    public static  Intent newIntent(Context packageContext){
        Intent intent = new Intent(packageContext , DecodeFragment.class) ;
        return intent ;
    }

    static class MyHandler extends Handler {

        private WeakReference<DecodeFragment> activityReference;

        public MyHandler(DecodeFragment activity) {
            activityReference = new WeakReference<DecodeFragment>(activity);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case PARSE_BARCODE_SUC: // 解析图片成功
                    DecodeFragment decodeActivity = (DecodeFragment) activityReference.get();
                    if(decodeActivity != null) {
                        decodeActivity.dealParseBarcodeSuc((String) msg.obj);
                    }
                    break;

                case PARSE_BARCODE_FAIL:// 解析图片失败
                    decodeActivity = (DecodeFragment) activityReference.get();
                    if(decodeActivity != null) {
                        decodeActivity.dealParseBarcodeFail();
                    }
                    break;
    ...
    protected void dealParseBarcodeFail(){
        Toast.makeText(this.getContext(), "解析图片失败",
                Toast.LENGTH_SHORT).show();
    }

    protected void dealParseBarcodeSuc( String msg){
        //子类覆盖 执行跳转逻辑
        Toast.makeText(this.getContext(),
                "解析成功，结果为：" + msg , Toast.LENGTH_SHORT).show();
    }
    protected void dealParseBarcodeSuc(BarcodeFormat barcodeFormat , String msg){
        //子类覆盖 执行跳转逻辑
        Toast.makeText(this.getContext(),
                "解析成功，结果为：" + msg , Toast.LENGTH_SHORT).show();
    }
...
//        setContentView(R.layout.activity_common_decode);
...
//        findViewById(R.id.capture_button_cancel).setOnClickListener(this);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
         fragmentView = inflater.inflate(R.layout.activity_common_decode, null);
         return fragmentView ;
    }
    ...
    public void onResume() {
        ...
        scannerView = (ScannerView) fragmentView.findViewById(R.id.capture_viewfinder_view);
        ...
        SurfaceView surfaceView = (SurfaceView) fragmentView.findViewById(R.id.capture_preview_view); // 预览
    ...
    public void onPause() {
        ...
        if (!hasSurface) {
            SurfaceView surfaceView = (SurfaceView) fragmentView.findViewById(R.id.capture_preview_view);
            SurfaceHolder surfaceHolder = surfaceView.getHolder();
...
//                Toast.LENGTH_SHORT).show();
        dealParseBarcodeSuc( rawResult.getBarcodeFormat() ,ResultParser.parseResult(rawResult).toString()) ;
    }
    ...
    private void displayFrameworkBugMessageAndExit() {
        ...
        builder.setPositiveButton("确定", new FinishListener(this.getActivity()));
